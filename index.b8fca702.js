...
function handleKeyDown(keyEvent) {
    ...
    if (didTilesMove) {
        ...
        if (currentGameStatus === Game.Status.lose) handleLostGame();
        else if (currentGameStatus === Game.Status.win) handleWonGame();
    }
}
 ...
 */ class Game {
   ...
   */ restart() {
    ...
    createRandomTile() {
        ...
        while(!this.state[row].includes(0))// Keep re-assigning a new row until it has a 0
        row = this.randomNumber(3);
        // Assign a random tile
        let cell = this.randomNumber(3);
        // If the tile's value is 0
        while(this.state[row][cell] !== 0)// Keep re-assigning a new cell until it has a 0
    ...
    printTiles() {
        ...
        for(let i = 0; i < flatState.length; i++){
            ...
            if (currentState > 0) // If current cell exists
            {
